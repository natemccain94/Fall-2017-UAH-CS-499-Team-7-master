...
public partial class agentMain : System.Web.UI.Page
{
    ...
    protected void Bind_gridview(string option)
    {
        ...
        bool agent_id_flag = false;
        int agent_id = 0;
        bool agency_id_flag = false;
        int agency_id = 0;

        counter = 0;
        HttpCookie _userInfo = Request.Cookies["_userInfo"];
        agent_id_flag = int.TryParse(_userInfo["AgentID"], out agent_id);
        agency_id_flag = int.TryParse(_userInfo["AgencyID"], out agency_id);
        con.Open();
        SqlCommand cmd = con.CreateCommand();                   //create object of command called cmd
        cmd.CommandType = CommandType.Text;
        if (option == "all")
        {
            cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id;
        }
        else if (option == "street")
        {
            // columnname like '%Moses%' and columnname like'%Robi%'
            cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id + "and listing_street LIKE '%" + txtSearch.Text + "%'";
        }
        else if (option == "zipcode")
        {
            ...
            if (result)
            {
                cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id + "and listing_zip=" + zipcode;
            }
            else
            {
                cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id;
        ...
        else if (option == "price")
        {
            ...
            {
                cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id + "and listing_price >=" + min + "and listing_price <=" + max;
            }
            else if (min_flag && max_flag == false)
            {
                cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id + "and listing_price >=" + min;
            }
            else if (max_flag && min_flag == false)
            {
                cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id + "and listing_price <=" + max;
            }
            else
            {
                cmd.CommandText = "Select * FROM listing WHERE agency_id=" + agency_id;
